# agent86 Documentation

## Project Overview

agent86 is a self-contained 8086 assembler, disassembler, and emulator in a single C++ file (`main.cpp`). It produces flat `.COM` binaries and is designed for AI agents — every output is machine-readable JSON with actionable diagnostics.
...
If adding an instruction, it needs to be added in four places: the ISA database, the encoder, the decoder, and the emulator.
...
| `--dos-root <dir>` | **Mount a host directory** as the DOS filesystem root (C:\). Enables file I/O functions (open, read, write, close, directory listing). All paths are sandboxed within this directory. Without this flag, file I/O INT 21h functions return CF=1 (error). | Off |
...
| Flag | Purpose | Output |
|---|---|---|
| `--help` | **List all help topics.** JSON index of every queryable topic. | JSON topic listing |
| `--help <topic>` | **Detailed help for a specific topic.** Content, examples, and related topics. Case-insensitive. | JSON topic detail |

The `--help` system provides queryable, structured access to all documentation — designed for agents with limited context windows that can't process the full manual. Topics cover every CLI flag, assembly syntax, directives, macros, structs, the full instruction set, addressing modes, all emulated interrupts, JSON output schemas, and diagnostic references. Help content is stored in `agent86.hlp` alongside the executable.
...
- `mouse` *(only when `--mouse` is used)*: `{"x": N, "y": N, "buttons": N, "visible": bool}`. Final mouse driver state.
...
- `filesOpened`: Number of files opened via AH=3Ch/3Dh.
- `filesClosed`: Number of files closed via AH=3Eh.
- `bytesRead`: Total bytes read via AH=3Fh.
- `bytesWritten`: Total bytes written via AH=40h.
- `dirSearches`: Number of FindFirst (AH=4Eh) calls.
- `errors`: Number of file I/O errors (invalid handles, path not found, etc.).
...
- `finalState.mouse` *(only when `--mouse` is used)*: `{"x": N, "y": N, "buttons": N, "visible": bool}`.
...
| `19h` | Get current disk | Returns current drive in AL (0=A, 2=C, default=2). |
| `25h` | Set interrupt vector | Stub — silently ignored. Programs use this to set INT 24h error handler. |
...
| `62h` | Get PSP segment | Returns BX=0 (PSP at segment 0). |

#### Memory Management

The emulator provides a 1MB address space. The COM program occupies segment 0 (64KB). Allocatable memory spans segments 0x1000–0x9FFF (576KB). Allocation is a linear bump allocator.

| AH | Function | Behavior |
|---|---|---|
| `48h` | Allocate memory | Allocates BX paragraphs (16 bytes each). Returns segment in AX, CF=0. On failure: CF=1, AX=8, BX=largest free. |
| `49h` | Free memory | Frees block at segment ES. CF=0 on success. CF=1, AX=9 if invalid block. |
| `4Ah` | Resize memory block | Resizes block at ES to BX paragraphs. CF=0 on success. CF=1, AX=8, BX=largest free on failure. |

#### File Handle I/O (requires `--dos-root`)

| AH | Function | Behavior |
|---|---|---|
| `3Ch` | Create file | Creates/truncates file at DS:DX path. Returns handle in AX. CF=1 on error (AX=error code). |
| `3Dh` | Open file | Opens file at DS:DX. AL=access mode (0=read, 1=write, 2=r/w). Returns handle in AX. CF=1 on error. |
| `3Eh` | Close file | Closes handle in BX. Works for both file and device handles. CF=1 if invalid handle. |
| `3Fh` | Read file/device | Reads CX bytes from handle BX into DS:DX buffer. Returns actual bytes read in AX. Handle 0 (STDIN) reads from `--input`. CF=1 on error. |
| `40h` | Write file/device | Writes CX bytes from DS:DX buffer to handle BX. Returns bytes written in AX. Handles 1-2 (STDOUT/STDERR) write to output buffer. CF=1 on error. |
| `42h` | Seek (lseek) | Seeks handle BX. AL=origin (0=start, 1=current, 2=end), CX:DX=offset. Returns new position in DX:AX. CF=1 on error. |
| `43h` | Get/set file attributes | AL=0: returns file attributes in CX (0x20=archive, 0x10=directory). AL=1: stub (succeeds silently). CF=1 if file not found. |
| `44h/00` | IOCTL get device info | Returns device info word in DX for handle BX. Bit 7=is device. CF=1 if invalid handle. |
| `57h` | Get file date/time | Returns file timestamp for handle BX: CX=time, DX=date (packed DOS format). |

#### Directory Operations (requires `--dos-root`)

| AH | Function | Behavior |
|---|---|---|
| `1Ah` | Set DTA | Sets Disk Transfer Area address to DS:DX. Used by FindFirst/FindNext. Default: PSP:0080h. |
| `2Fh` | Get DTA | Returns DTA address in ES:BX. |
| `3Bh` | Change directory | Changes current directory to path at DS:DX. CF=1 if path not found. |
| `47h` | Get current directory | Writes current directory string to DS:SI buffer (no leading backslash, null-terminated). |
| `4Eh` | FindFirst | Finds first file matching pattern at DS:DX with attribute mask CX. Writes match to DTA. CF=1 if no files found (AX=18). |
| `4Fh` | FindNext | Finds next file from previous FindFirst search. Writes match to DTA. CF=1 when no more files (AX=18). |
...
Handles 0-4 are pre-populated device handles:

| Handle | Device | Read behavior | Write behavior |
|--------|--------|--------------|----------------|
| 0 | STDIN | Reads from `--input` | Discarded |
| 1 | STDOUT | Returns 0 bytes | Appends to output buffer + VRAM |
| 2 | STDERR | Returns 0 bytes | Appends to output buffer + VRAM |
| 3 | AUX | Returns 0 bytes | Discarded |
| 4 | PRN | Returns 0 bytes | Discarded |

File handles 5+ are allocated by `3Ch` (Create) and `3Dh` (Open).

#### DTA Layout (FindFirst/FindNext)

The Disk Transfer Area is a 43-byte structure written by FindFirst (4Eh) and FindNext (4Fh):

| Offset | Size | Field |
|--------|------|-------|
| `00h` | 21 | Reserved (internal search state) |
| `15h` | 1 | File attribute byte |
| `16h` | 2 | File time (packed DOS format) |
| `18h` | 2 | File date (packed DOS format) |
| `1Ah` | 4 | File size (DWORD, little-endian) |
| `1Eh` | 13 | Filename (8.3 format, ASCIIZ) |

#### Path Sandboxing

All DOS paths are resolved relative to the `--dos-root` directory. The emulator:
1. Replaces `\` with `/` in DOS paths
2. Strips drive letters (e.g., `C:`)
3. Prepends the current directory for relative paths
4. Joins with the root path and canonicalizes
5. Verifies the result is within the root directory (rejects `..` traversal attempts)

Path traversal attempts (e.g., `..\..\Windows\System32`) return error code 3 (path not found).

### INT 33h — Mouse Services

Enabled by the `--mouse` CLI flag. When `--mouse` is not specified, INT 33h function 0000h returns AX=0 (driver not installed) and all other functions are no-ops. INT 33h dispatches on the full AX register, not just AH.

| AX | Function | Behavior |
|---|---|---|
| `0000h` | Reset / detect | If installed: AX=FFFFh, BX=3 (3-button mouse). Resets position to center of range, clears button counters. If not installed: AX=0, BX=0. |
| `0001h` | Show cursor | Sets cursor visible (tracked in JSON output). |
| `0002h` | Hide cursor | Sets cursor hidden. |
| `0003h` | Get position & buttons | BX=button state, CX=X position, DX=Y position. |
| `0004h` | Set position | CX=X, DX=Y. Values clamped to current range. |
| `0005h` | Get button press info | BX (in)=button index. Returns: AX=current buttons, BX=press count (then reset), CX=last press X, DX=last press Y. |
| `0006h` | Get button release info | BX (in)=button index. Returns: AX=current buttons, BX=release count (then reset), CX=last release X, DX=last release Y. |
| `0007h` | Set horizontal range | CX=min X, DX=max X. Position clamped if out of new range. |
| `0008h` | Set vertical range | CX=min Y, DX=max Y. Position clamped if out of new range. |
| `000Bh` | Get motion counters | CX=0, DX=0 (static emulation — no motion). |
| `000Ch` | Set event handler | Accepted and silently ignored (no async events in emulator). |

Any other AX value is logged in `skipped[]`.

**Example — detect mouse and read position:**
...
The `--events` flag scripts a timeline of user interactions — keyboard input, mouse state changes, and snapshots — triggered by what the program does. This enables testing interactive programs that interleave keyboard and mouse interaction.
...
- `read` and `poll` triggers fire inside the interrupt handler, so injected state is available to the current read/poll.
- `tick`, `int21`, and `int33` triggers fire after each cycle in the main loop.
- `--events` is composable with `--input` and `--mouse` (events can override mouse state set by `--mouse`).
...
**Example — debugging a mouse-driven menu:**
```json
[
...
```
This clicks at (320,100) on the 3rd mouse poll, releases on the 8th, then types "Y" on the first keyboard read.
...
```asm
; main.asm — pull in data and subroutines from separate files
ORG 100h
    MOV AH, 9
    MOV DX, msg
    INT 21h
...
INCLUDE constants.inc       ; bare filename
```

All three quoting styles are equivalent. Relative paths are resolved from the directory of the file containing the INCLUDE, not from the working directory. When using `--agent` or `--run-source` mode, the JSON output includes `"file"` and `"sourceLine"` fields in diagnostics, symbols, and listing entries so errors can be traced back to their original source file.

### Macros

Define reusable code patterns with `MACRO`/`ENDM`. Macros are expanded as a text-level preprocessor step after `INCLUDE` expansion but before the two-pass assembler. The pipeline is: `expandIncludes()` → `expandMacros()` → two-pass assembly.
...
**Syntax:** `name MACRO [param1, param2, ...]` ... `ENDM`. Parameters are comma-separated identifiers. Substitution is case-insensitive, respects word boundaries (param `x` won't match inside `AX`), and skips strings and comments. Macro names are case-insensitive.

Use `&` for concatenation — the `&` is consumed, joining parameter text with surrounding characters:
...
`LOCAL` must appear before instructions in the macro body. Multiple locals: `LOCAL a, b, c`.
...
Count must be a **numeric literal** (decimal, hex `0Ah`, binary `100b`); EQU constants are not supported. `REPT 0` emits nothing. REPT blocks can be nested.
...
List items are enclosed in `<>` and comma-separated. An empty list `<>` emits nothing.
...
Expansion iterates until stable (up to 10,000 iterations). Self-recursive macros hit this limit and produce an error.

A label before an invocation is preserved: `start: PrintChar 'A'` places the label on its own line, then the expansion.

**Restrictions:** Macro names cannot shadow reserved words (instructions, registers, directives). Redefining a macro emits a warning. Argument count mismatches emit warnings (missing args become empty, extras ignored).
...
| `"Conditional jump auto-promoted"` | Jcc target exceeded short range; assembler emitted inverted-Jcc + near JMP (5 bytes) | The expanded encoding size (INFO-level, not an error) |
