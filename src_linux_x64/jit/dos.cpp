...
static void writeDTARecord(CPU8086& cpu, uint32_t dta_phys,
    ...
    uint16_t dosTime = packDOSTime(t->tm_hour, t->tm_min, t->tm_sec);
    uint16_t dosDate = packDOSDate(t->tm_year + 1900, t->tm_mon + 1, t->tm_mday);
        ...
        case 0x40: {
            ...
            if (bx >= 5 && bx < 20 && dos.handles[bx]) {
                if (cx == 0) {
                    ...
                    (void)ftruncate(fileno(dos.handles[bx]), pos);
                    cpu.regs[R_AX] = 0;
                } else {
                    uint32_t phys = physAddr(cpu.sregs[S_DS], dx);
